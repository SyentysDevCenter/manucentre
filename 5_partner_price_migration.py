...
product_tag = odoov13.env['product.tags']
product_tag_ids = product_tag.search([])
product_tag_data = product_tag.read(product_tag_ids, ['old_id'])
dict_product_tag = {part['old_id']: part['id'] for part in product_tag_data}
...
for p in product_tags:
    if dict_product_prod.get(p[0], False) and dict_product_tag.get(p[1], None):
        data = (
            dict_product_tag.get(p[1], None),
            dict_product_prod.get(p[0], None),
...
product_price = []
standard_price_field = odoov13.execute_kw('ir.model.data', 'get_object_reference',
        ...
        if p[1] not in prod:
            ...
            if product_id:
                product_price.append(('standard_price', 'product.product,' + str(product_id), company,
                                      standard_price_field[1], float(p[0]), 'float'))
